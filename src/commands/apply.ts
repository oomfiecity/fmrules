 ...
 *
 * Fastmail's rule engine only fires at delivery (SPEC(10).md §11.3). This
 * command is the external tool the spec anticipated: it evaluates each
 ...
 *
 * Mutation safety: Fastmail's Email/set replaces whole mailboxIds/keywords
 * maps, so current state is fetched per message and complete desired maps
 * are written. Use --dry-run to see the plan without changing anything;
 * --yes is required to actually mutate.
 */
...
const builder = (y: Argv) =>
  y.options({
    ...
    after: { type: 'string', describe: 'Only apply to messages received after this date (YYYY-MM-DD)' },
    before: { type: 'string', describe: 'Only apply to messages received before this date (YYYY-MM-DD)' },
...
const handler: CommandModule['handler'] = async (argv) => {
  ...
  try {
    ...
    // Current state for every claimed message (whole-map write-back requires it).
